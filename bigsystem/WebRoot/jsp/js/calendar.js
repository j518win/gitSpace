document.write("<div id=meizzCalendarLayer style='position: absolute; z-index: 9999; width:154; height:211; display:none'>");
...
function writeIframe(){
    ...
    "table#tableMain{ width:152; height:180;}"+
    "table#tableWeek td{ color: "+ WebCalendar.lightColor +";}"+
    "table#tableDay  td{ font-weight: bold;}"+
    "td#meizzYearHead, td#meizzYearMonth{color: "+ WebCalendar.wordColor +"}"+
    ".out { text-align: center; border-top: 1px solid "+ WebCalendar.DarkBorder +"; border-left: 1px solid "+ WebCalendar.DarkBorder +";"+
	"border-right: 1px solid "+ WebCalendar.lightColor +"; border-bottom: 1px solid "+ WebCalendar.lightColor +";}"+
    ".hms { text-align: center; border-top: none "+ WebCalendar.lightColor +"; border-left: 1px solid "+ WebCalendar.lightColor +";"+
	"border-right: 1px solid "+ WebCalendar.darkColor +"; border-bottom: 1px solid "+ WebCalendar.darkColor +";}"+
    ...
    "<select name=tmpMonthSelect onblur='parent.hiddenSelect(this)' style='z-index:1; position:absolute;top:3;left:80;display:none'"+
    " onchange='parent.WebCalendar.thisMonth=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
	"<select name=tmpHourSelect onblur='parent.hiddenSelect(this)' style='width:50;z-index:1;position:absolute;top:170;left:3;display:none'"+
    "	onchange='parent.WebCalendar.thisHour =this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
	"<select name=tmpMinuteSelect onblur='parent.hiddenSelect(this)' style='width:50;z-index:1; position:absolute;top:170;left:53;display:none'"+
    "	onchange='parent.WebCalendar.thisMinute=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
	"<select name=tmpSecondSelect onblur='parent.hiddenSelect(this)' style='width:50;z-index:1; position:absolute;top:170;left:103;display:none'"+
    "	onchange='parent.WebCalendar.thisSecond=this.value; parent.hiddenSelect(this); parent.writeCalendar();'></select>"+
    "<table id=tableMain class=bg border=0 cellspacing=2 cellpadding=0>"+
    "<tr><td width=150 height=19 bgcolor='"+ WebCalendar.lightColor +"'>"+
    "    <table width=150 id=tableHead border=0 cellspacing=1 cellpadding=0><tr align=center>"+
    ...
    "</td></tr><tr><td height=20><table id=tableWeek border=1 width=150 cellpadding=0 cellspacing=0 ";
    ...
    "</td></tr><tr><td valign=top width=150 bgcolor='"+ WebCalendar.lightColor +"'>"+
    "    <table id=tableDay height=120 width=150 border=0 cellspacing=1 cellpadding=0>";
    ...
    "</td></tr><tr><td height=20 width=150 bgcolor='"+ WebCalendar.lightColor +"'>"+
    "    <table border=0 cellpadding=1 cellspacing=0 width=150>";
	/* ----------*/ 
	strIframe += "	<tr><td class=hms width=50 id=meizzHourHead ";
	if (WebCalendar.thisPrecision >= 1){
		strIframe += " title='点击此处选择小时' onclick='parent.funHourSelect(parseInt(this.innerText, 10))'"+
			" onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
   			" onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'>";
	}
	strIframe += "</td><td width=50 class=hms id=meizzHourMinute ";
	if (WebCalendar.thisPrecision >= 2 ){
		strIframe += " title='点击此处选择分钟' onclick='parent.funMinuteSelect(parseInt(this.innerText, 10))'"+
			" onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
			" onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'>";
	}
	strIframe += "</td><td width=50 class=hms id=meizzHourSecond ";
	if (WebCalendar.thisPrecision >= 3 ){
		strIframe += " title='点击此处选择秒钟' onclick='parent.funSecondSelect(parseInt(this.innerText, 10))'"+
			" onmouseover='this.bgColor=parent.WebCalendar.darkColor; this.style.color=parent.WebCalendar.lightColor'"+
			" onmouseout='this.bgColor=parent.WebCalendar.lightColor; this.style.color=parent.WebCalendar.wordColor'>";
	}
	strIframe += "</td></tr>";
	...
	/* ----------*/
	strIframe +="    <tr><td><input name=prevYear title='向前翻 1 年&#13;快捷键：↑' onclick='parent.prevY()' type=button value='&lt;&lt;'"+
...
function WebCalendar() {
	...
	this.thisMinute = new Date().getMinutes();  //定义分中的变量的初始值
	...
	this.darkColor  = "#1E6AC3";    //控件的暗色FF6347
...
function calendar(){
	...
	if (arguments.length == 0){
		...
		WebCalendar.objExport = eval(arguments[0]);
		WebCalendar.thisPrecision = arguments[1];
	}
    ...
    if  (!WebCalendar.timeShow){
		...
		switch(WebCalendar.thisPrecision){
		case 1:
			WebCalendar.dateReg = /^(\d{1,4})(-|\/|.)(\d{1,2})\2(\d{1,2}) (\d{1,2})\S*$/;
			break;
		case 2:
			WebCalendar.dateReg = /^(\d{1,4})(-|\/|.)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2})\S*$/;
			break;
		case 3:
			WebCalendar.dateReg = /^(\d{1,4})(-|\/|.)(\d{1,2})\2(\d{1,2}) (\d{1,2}):(\d{1,2}):(\d{1,2})$/;
			break;
    ...
    try{
        if (WebCalendar.objExport.value.trim() != ""){
			...
			if (WebCalendar.dateStyle == null) {
                ...
                alert("原输入框里的日期有错误,可能与你定义的显示时分秒有冲突！\n请注意格式,如：2007-02-12 13:35:08");
...
{
    var m = isNaN(parseInt(WebCalendar.thisMonth, 10)) ? new Date().getMonth() + 1 : parseInt(WebCalendar.thisHour);
...
{
    var m = isNaN(parseInt(WebCalendar.thisMonth, 10)) ? new Date().getMonth() + 1 : parseInt(WebCalendar.thisMinute);
    var e = WebCalendar.iframe.document.forms[0].tmpMinuteSelect;
    for (var i=0; i<60; i++) e.options.add(new Option(i +"分", i));
    e.style.display = ""; e.value = m; e.focus(); window.status = e.style.top;
}
function funSecondSelect() //秒钟的下拉框
{
    var m = isNaN(parseInt(WebCalendar.thisMonth, 10)) ? new Date().getMonth() + 1 : parseInt(WebCalendar.thisSecond);
...
function writeCalendar() //对日历显示的数据的处理程序
{
    ...
    if (!(y<=9999 && y >= 1000 && parseInt(m, 10)>0 && parseInt(m, 10)<13 && parseInt(d, 10)>0)){
        alert("对不起，你输入了错误的日期！");
        var dt = new Date();
		WebCalendar.thisYear   = dt.getFullYear();
        WebCalendar.thisMonth  = dt.getMonth()+ 1;
        WebCalendar.thisDay    = dt.getDate();
		WebCalendar.thisHour   = dt.getHours();
		WebCalendar.thisHour   = dt.getHours();
        WebCalendar.thisMinute = dt.getMinutes();
        WebCalendar.thisSecond = dt.getSeconds();
	}
    HH = WebCalendar.thisHour;
    MM = WebCalendar.thisMinute;
    SS = WebCalendar.thisSecond;
    WebCalendar.iframe.meizzYearHead.innerText  = y +" 年";
    WebCalendar.iframe.meizzYearMonth.innerText = m + " 月";
	WebCalendar.iframe.meizzHourHead.innerText  = parseInt(HH, 10) +" 时";
    WebCalendar.iframe.meizzHourMinute.innerText = parseInt(MM, 10) +" 分";
	WebCalendar.iframe.meizzHourSecond.innerText = parseInt(SS, 10) +" 秒";
...
function returnDate() //根据日期格式等返回用户选定的日期
{
    if(WebCalendar.objExport)
    {
        ...
        if(WebCalendar.timeShow)
        {
			...
			m = WebCalendar.thisMinute;
			s = WebCalendar.thisSecond;
			if (WebCalendar.thisPrecision == 3){
				returnValue += flag ? " "+ appendZero(h) +":"+ appendZero(m) +":"+ appendZero(s) : " "+ h +":"+ m +":"+ s;
			}else if (WebCalendar.thisPrecision == 2){
				returnValue += flag ? " "+ appendZero(h) +":"+ appendZero(m) + ":00" : " "+ h +":"+ m + ":00";
			}else if (WebCalendar.thisPrecision == 1){
				returnValue += flag ? " "+ appendZero(h) + ":00:00": " "+ h + ":00:00";
			}else{
			}
        }
