...
_p.insertItemAt = function (item, n){
	...
	item.load();	
	item.unSelect();	
	this.selectItemByNumber(item.getNumber())
}
...
_p.initTable = function(){
	...
	var str = '<table width="' 
			+ this._width + '" height="' 
			+ this._height + '" border="0" cellspacing="0" cellpadding="0" id="table1">'
...
//	var obj = tpCreateElementWithHTML(str);
	str = '<iframe name="' 
		+ this._name + '" src="about:blank" scrolling="auto" width="100%" height="100%" frameborder="0"></iframe>'
...
_p._initIframe = function(){
	var str;
	//str = '<iframe style="visibility:hidden;position:absolute;z-index:-10;" name="' 
	//	+ this._name + '" src="" scrolling="auto" width="100%" height="100%" frameborder="0"></iframe>'
	str = '<iframe style="display:none;position:absolute;z-index:-10;" name="' 
		+ this._name + '" src="" scrolling="auto" width="100%" height="100%" frameborder="0"></iframe>'
	var ifr = tpCreateElementWithHTML(str);
//	this._frameCell.appendChild(ifr);
	this._iframe = ifr;
...
_p.getSelectedHtml = function(){
	var str = "";
	str += '<label style="cursor:hand" title="双击刷新" unselectable="on" class="tabpane-selected">' + this._text+' ' + '<img src="images/close_tab.gif" width="8" style="cursor:hand" ></label>';
	return str;
}
...
_p.getUnSelectedHtml = function(){
	var str = "";
	str += '<label style="cursor:hand" title="双击刷新" unselectable="on" style="cursor:hand" class="tabpane-unselected">' + this._text +' ' + '<img src="images/close_tab.gif"  width="8" style="cursor:hand" ></label>';
	return str;
}
...
function _tiClickEvent(e){
	...
	{
	  e.srcElement.parentElement.TabItem._pane.removeItem(e.srcElement.parentElement.TabItem)	  
	  return;	 
	}
